// Gap 1 â€” Form W-2c (Corrected Wage and Tax Statement) renderer.
//
...
// "Previously reported" column and a "Correct information" column â€”
...
export interface W2cPdfData {
  ...
  amounts: W2cAmounts;
  /** Free-text reason â€” appears on the form so the recipient knows why. */
...
function drawIdBlock(doc: PDFKit.PDFDocument, data: W2cPdfData): void {
  ...
  // Original W-2 control number (Box d on the W-2c â€” different from Box d
...
function drawStateLines(doc: PDFKit.PDFDocument, amounts: W2cAmounts): void {
  ...
  doc
    ...
    .fontSize(7)
    .fillColor('#000')
    .text('15-17  State wages / tax (previous â†’ corrected)', 36, top - 12, {
      width: 540,
    });

  if (states.size === 0) {
    cell(doc, 36, top, 540, 26, '', 'â€” No state withholding â€”');
    return;
  }
  ...
  for (const state of [...states].sort()) {
    ...
    doc
      ...
      .text(
        `wages ${usd(prevWages)} â†’ ${usd(currWages)}    tax ${usd(prevTax)} â†’ ${usd(currTax)}`,
        80,
        y + 8,
        { width: 460 },
      );
...
function drawFooter(doc: PDFKit.PDFDocument, data: W2cPdfData): void {
  doc
    ...
    .fillColor('#666')
    .text(
      `Form W-2c (${data.taxYear})  Â·  ID ${data.meta.formId.slice(0, 8)}  Â·  ` +
        `Amends ${data.meta.originalFormId.slice(0, 8)}  Â·  ` +
...
function valueCell(
  ...
  doc
    ...
    .text(value || 'â€”', x + 4, y + 4, { width: w - 8 });
  doc.fillColor('#000');
}

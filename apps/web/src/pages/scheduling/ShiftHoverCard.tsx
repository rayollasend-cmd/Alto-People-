...
export function ShiftHoverCard({
  ...
  return createPortal(
    <div
      ...
      className="rounded-lg border border-navy-secondary bg-navy elev-3 overflow-hidden flex flex-col animate-fade-in"
    >
      ...
      <div className="px-4 py-3 space-y-2 text-sm overflow-auto">
        ...
        {shift.payRate != null && (
          <div className="text-xs text-silver/70 tabular-nums">
            {fmtPayRate(shift.payRate, 'HOURLY')} · projected{' '}
            {fmtMoney((shift.payRate * (shift.scheduledMinutes ?? 0)) / 60)}
          </div>
        )}
      </div>

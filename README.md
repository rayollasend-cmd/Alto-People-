...
> **Status:** Phase 18 — Multi-jurisdiction payroll tax + paystub PDF. The placeholder flat-rate withholding from Phase 8 is gone; payroll now runs the real IRS Pub 15-T 2024 percentage method (per filing status), 6.2% Social Security with the $168,600 wage-base cap, 1.45% Medicare with the 0.9% additional surcharge over $200k YTD, and per-state SIT (real CA + NY brackets, hard-zero for the nine no-SIT states, 4% fallback for the rest). Employer-side FICA + Medicare matching, FUTA, and SUTA are computed and stored separately so finance can see the full burdened cost. Paystubs render as PDFs on demand — `GET /payroll/items/:itemId/paystub.pdf` — with a `paystubHash` stamp so any drift in the immutable record is detectable.

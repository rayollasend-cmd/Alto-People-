...
> **Status:** Phase 23 — Per-state labor policy templates. New `lib/stateLaborPolicy.ts` carries 2024 wage/OT/break/sick-leave defaults for the 15 highest-volume hospitality states (CA, NY, IL, MA, NJ, PA, WA, CO, AZ, GA, NC, VA, FL, TX, OR) with a FEDERAL fallback. The clock-out anomaly engine now consults the associate's state — a 9-hour day in CA correctly fires `OVERTIME_UNAPPROVED` (daily-OT threshold = 8h); the same day in FL/TX does not. Meal-break minimums also flex per state (CA = 30min after 5h; PA defers to federal).

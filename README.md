...
> **Status:** Phase 19 — E-signature with PDF + audit trail. The Phase 4 `Signature` table (typed-name proof only) now backs a full server-rendered signing flow. HR creates an `EsignAgreement` against an Application (optionally linked to an `E_SIGN` checklist task); the associate signs by typing their name; the server renders a PDF with the agreement body, the typed signature in script style, and an audit panel embedding signer name/email/IP/UA/signedAt and a sha256 of the rendered bytes. The PDF is stored under the document vault (kind `SIGNED_AGREEMENT`, status `VERIFIED`), the linked task auto-completes, and `Signature.pdfHash` lets future downloads verify the bytes never drifted.

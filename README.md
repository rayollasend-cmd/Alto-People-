...
> **Status:** Phase 21 — Associate-facing I-9 React UI. The `I9_VERIFICATION` checklist task is now a real (non-stub) page at `/onboarding/me/:applicationId/tasks/i9_verification`. Section 1 is a guided attestation form that reveals A-Number and work-auth-expiry fields when the associate picks `LAWFUL_PERMANENT_RESIDENT` or `ALIEN_AUTHORIZED_TO_WORK`, with a typed-name signature. Documents upload via `<input type="file" accept="image/*" capture="environment">` so a phone opens the camera directly; on desktop it falls back to the file picker. Section 2 status is shown read-only on the associate side; HR completes verification through the existing Phase 20 endpoint.

...
> **Status:** Phase 16 — Associate invitation flow. HR creating an Application now provisions an `INVITED` User with a SHA-256-hashed `InviteToken` (7-day TTL) and queues a welcome EMAIL with a magic link. Real Resend delivery activates when `RESEND_API_KEY` + `RESEND_FROM` are set; without them the body (with the link) prints to the API console so the dev flow stays fully exercisable. The `/accept-invite/:token` page validates the token, sets the password, marks the user `ACTIVE`, and issues the session cookie in a single transaction.

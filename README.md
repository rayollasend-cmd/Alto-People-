...
> **Status:** Phase 22 — Disbursement adapter pattern + bulk paystub ZIP. The Phase 8 inline `STUB-...` ref generator is gone; disbursement now flows through a pluggable `DisbursementAdapter` (`StubAdapter` is the default, `WiseAdapter` and `BranchAdapter` are scaffolded for one-line provider switches). Every call appends a `PayrollDisbursementAttempt` row regardless of outcome — finance can now reconstruct the full provider-call history per item. A `FAILED` provider response holds that single item (`HELD` status with `failureReason`) without failing the whole batch; the run only flips to `DISBURSED` when every item succeeded. New `GET /payroll/runs/:runId/paystubs.zip` streams a ZIP of every paystub PDF in the run for HR/Finance bulk download.

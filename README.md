...
> **Status:** Phase 17 — Invitation lifecycle hardened. A 48-hour reminder sweep (gated by `INVITE_REMINDER_INTERVAL_SECONDS`) finds INVITED users who never accepted, rotates their token, and emails a fresh magic link — once. HR can also trigger a manual rotation through `POST /onboarding/applications/:id/resend-invite` (always rotates, kills the old link). The previous Phase 16 invitation flow (provision INVITED user → SHA-256-hashed `InviteToken` → welcome email → `/accept-invite/:token` sets password and issues session cookie) is unchanged.
